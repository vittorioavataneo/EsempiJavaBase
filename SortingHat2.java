...
public class SortingHat2{
    public static void main(String[] args){
        ...
        for(int i=0; i<pp; i++){
            ...
            if(!isFull(houses, capienzaMax, idCasa)){
                int nCasa = addToHouse(s,idCasa,houses);
                System.out.println("Studente messo in " + nCasa);
            }else {
                do {
			        idCasa = (idCasa + 1)%NumeroCase; 
		        }while(isFull(houses, capienzaMax, idCasa)); 
                int nCasa = addToHouse(s,idCasa,houses); 
				System.out.println("studente inserito nella colonna " + nCasa);     
		   	}
    ...
    public static int assegnazioneCasa(long x/*, String[][] houses, String nome, int capienzaMax*/){
        int casa=0;
        //String[][] house = houses[casa][capienzaMax];
        if(x % 4 == Gryffindor){
            casa=casa;
            //houses[casa][capienzaMax]=[casa][nome];
        } 
    ...
    public static int addToHouse(String nome, int idCasa, String [][] houses){
		String[] house = houses[idCasa];
		int col = 0;
		while(house[col] != null){ //mettere idCasa?
		  col++;				 
		}
		house[col] = nome;
		return col;
